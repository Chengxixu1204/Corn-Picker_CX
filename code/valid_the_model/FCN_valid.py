...
if __name__ == "__main__":
    ...
    fcn_model.load_state_dict(torch.load( '/home/wanquan/Desktop/corn_project/result/fcn_corn_model_{}.pt'.format(43000)))
    #fcn_model =FCN16(1)
    ...
    for epo in range(20):
        ...
        label=np.loadtxt('/home/wanquan/Desktop/corn_project/data/maps/train/'+str(epo%20)+'.map').astype(np.float)
        
        label=label[np.newaxis,np.newaxis,:,:]
        label_tensor=torch.tensor(label,dtype=torch.float,requires_grad=False,device=dv)
        print label_tensor[0][0][0][0],label_tensor[0][0][0][1],label_tensor[0][0][0][2],label_tensor[0][0][0][3]

        imgB = cv2.imread('/home/wanquan/Desktop/corn_project/data/pictures/train/'+str(epo%20)+'.jpg')
        
        for h in range(len(imgB)):
            for w in  range(len(imgB[0])):

                v=output[0][0][h][w].item()

                b=(1-v)*255.0;
                g=(0.3-abs(v-0.5))*255.0;
                r=v*255.0;
                if h==0 and w==0:
                    print v,b,g,r
                if b<0:
                ...
                if h==0 and w==0:
                    print b,g,r
                if h==0 and w==0:
                    print imgB[h][w][0],imgB[h][w][1],imgB[h][w][2]
                imgB[h][w][0]=int(b*0.5+imgB[h][w][0]*0.5)
                #print imgB[h][w][0]
                imgB[h][w][1]=int(g*0.5+imgB[h][w][1]*0.5)
                imgB[h][w][2]=int(r*0.5+imgB[h][w][2]*0.5)
                if h==0 and w==0:
                    print imgB[h][w][0],imgB[h][w][1],imgB[h][w][2]
        cv2.imwrite('/home/wanquan/Desktop/corn_project/result/result_pictures/result_view'+str(epo%20)+'.jpg',imgB)
        #cv2.waitKey()  
        for h in range(len(imgB)):
            for w in  range(len(imgB[0])):
                v=output[0][0][h][w].item()
                ...
                imgB[h][w][1]=int(g)
                imgB[h][w][2]=int(r)
        cv2.imwrite('/home/wanquan/Desktop/corn_project/result/result_pictures/result_'+str(epo%20)+'.jpg',imgB)
